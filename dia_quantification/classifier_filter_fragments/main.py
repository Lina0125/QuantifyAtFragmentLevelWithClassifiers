...
def score(fp, out_path, Classifier, Scaler):
    ...
    print(f'Info: Predicting and calculating decision scores for {fp}', end='\n')
    y_scores = calculate_probability(classifier, feature_standard, target)

    # Calculate q values
    print(f'Info: Calculting q values for {fp}', end='\n')
...
def global_align(df, template):
    ...
    """
    Control FDR on protein and peptide level by aligning scored data back to pyProphet global model
    """
...
def filter_fragments(fp, template, method='top3', max_transition_qvalue=0.01, max_fragment_qvalue=0.01):

    """
    Align data back to global model to filter out unconfident proteins and peptides;
    Then it filters fragment ions according to its q values, and calculate MS1 intensity using optional methods;
    ...
    print(f'Info: FDR control on protein and peptide level for {fp}')
    ...
    print(f'Info: FDR control on fragment ion level for {fp}')
    data = data[data['FRAGMENT_QVALUE'] < max_fragment_qvalue]

    print(f'Info: Quantifing {fp} by {method} method')
...
def merge_runs(in_dir, template, method='mean', max_transition_qvalue=0.01, max_fragment_qvalue=0.01):
    ...
    print(f'Info: Merging {files[0]} to quantitative matrix')
    for file in files[1:]:
        ...
        print(f'Info: Merging {file} to quantitative matrix')
